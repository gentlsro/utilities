...
const { resolve } = (0, _kit.createResolver)("file:///home/jk/Projects/app-Root/libs/Utilities/modules/utilities.module.ts");
const currentDir = resolve('..');

function extractComparatorEnum(text) {
...
function extractTypeContent(content, typeName) {
...
(0, _kit.defineNuxtModule)({
  setup: async (_, nuxt) => {
    ...
    map((layer) => {
      ...
      componentPaths.push((0, _nodePath.relative)(process.cwd(), `${layer.cwd}/app/components/**/*.vue`));
      componentPaths.push((0, _nodePath.relative)(process.cwd(), `${layer.cwd}/app/libs/**/*.vue`));

      return { path: resolve(layer.cwd, configPath), isBase, cwd: layer.cwd };
    }).
    filter(({ path }) => (0, _nodeFs.existsSync)(`${path}.ts`));
...
`;
    ...
    map(({ path }) => {
      const fileContents = (0, _nodeFs.readFileSync)(`${path}.ts`, 'utf-8');
    ...
    .flatMap((typeContent) => typeContent.split('|').map((part) => part.trim())) // Split and trim each type
